...
var TravelLib = function () {
  return {
    ...
    createSection: function (id, done) {
      Travel.update({_id: id}, {
        $push: {sections: {}}
      }, function (err, numberAffected) {
        Travel.findOne({_id: id}, function (err, travel) {
          done(travel.sections[travel.sections.length-1]._id);
        });
      });
    },
    
    removeSection: function (id, sid, done) {
      Travel.update({_id: id}, {
        $pull: {
          sections: {_id: sid}
        }
